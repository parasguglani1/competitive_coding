...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll pr[n];
        pr[0] = 0;
        REP(i,1,n) pr[i] = pr[i-1] + a[i][i-1];
        ll dp[n][n], dpp[n];
        dp[0][0] = 0; dpp[0] = 0;
        REP(i,1,n)
        {
        	dp[i][0] = a[i][0] + pr[i-1];
        	dp[i][i-1] = a[i][i-1] + min(dpp[i-1], dp[i-1][max(i-2, 0LL)]);
        	REP(j,1,i-1) dp[i][j] = a[i][j] + dpp[j+1] + pr[i-1]-pr[j+1];
        	dpp[i] = dp[i][0];
        	REP(j,1,i-1) dpp[i] = min(dpp[i], dp[i][j]);
        }
        ll ans = dp[n-1][0] + a[n-1][n-2];
        REP(j,1,n-2) ans = min(ans, dp[n-1][j] + a[n-1][n-2]);
        cout<<ans + 2*(n-1)*d;
